...
class Movimiento ():
    def __init__(self, group = "arm", frame = "rbkairos_odom", endeff = "rbkairos_ur10_tool0"):
        ...
        #self.group.set_end_effector_link("rbkairos_spray_tool_end_link")    # creo que no haria falta porque en moveit ya lo hemos puesto como el ultimo link
    ...
    def set_joint_pose_case(self, case):
        ...
        if case.upper() == "INIT":
            ...
            #joint_values = [-3.14, -1.57, -2.88, 1.32, 2.02, 0.0]   # PLEGADO VERTICAL
    ...
    def move_case(self, case_code): # obtencion de puntos y orientaciones que rodean el objeto, segun el caso indicado por 'case_code'
        ...
        pub_msg = PoseStamped()
        pub_msg.header.frame_id="rbkairos_odom" # pondria el rbkairos_odom, pero con base_footprint esta funcionando bien
        pub_msg.header.stamp = rospy.Time.now()
        pub_msg.pose = waypoints[0]

        while self.pub_tool_goal.get_num_connections() < 1:
            rospy.sleep(0.5)
            rospy.logwarn("Esperando publisher del tool_goal")
...
rospy.init_node('arm_service_server_COMMANDER', log_level=rospy.DEBUG)
