...
import rospy
import sys
from rbkairos_navigation.srv import RBKairosPose, RBKairosPoseResponse, RBKairosPoseRequest #nuestro mensaje creado
from geometry_msgs.msg import PoseWithCovarianceStamped #el tipo de mensaje del topic /robot/amcl_pose
import rospkg
from shutil import rmtree   #para lo del archivo que guarda

class RBKairosPoseRetriever(object):
    def __init__(self, spots_file_name="spots_saved.yaml"):
        self.pose_service = rospy.Service('/record_spot', RBKairosPose , self.pose_service_callback) # crear el servicio
        ...
        spots_folder_path = os.path.join(rospack.get_path('my_navigation'), "spots")
        if os.path.exists(spots_folder_path):
            rmtree(spots_folder_path)   #si existe el directorio, lo elimina

        os.makedirs(spots_folder_path) #crea de nuevo la carpeta, ahora esta vacia
        print("Created folder=" + str(spots_folder_path))

        self.spot_file_path = os.path.join(spots_folder_path, spots_file_name)
        #esto de try-except no lo entiendo porque se supone que el archivo No existe, o sea la carpeta la acabas de crear vacia!
        try:
    ...
    def pose_service_callback(self, request):
        ...
        with open(self.spot_file_path, "a") as myfile:
            spot_pose = self.pose_now.pose.pose #dentro de este objeto tendremos .position y .orientation
